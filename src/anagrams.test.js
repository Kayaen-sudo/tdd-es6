...
// 'listens' 'silent' !anagrams
    it('returns false when strings have the same letters but in different quantities', () => {
        const expected = false;
        const actual = isAnagram('listens', 'silent');
        expect(actual).to.equal(expected);
    });

// 'conversation' 'voices rant on' anagrams
